            ...
            {benefits.map((benefit, index) => {
              ...
              return (
                <div
                  ...
                  className={`group relative cursor-pointer overflow-hidden rounded-[24px] bg-neutral-900/60 border border-neutral-800 p-5 md:p-6 text-left shadow-lg transition-all duration-200 ease-out focus:ring-2 focus:ring-[#16a34a] focus:outline-none transform ${isOpen ? 'border-[#16a34a] shadow-[0_0_24px_rgba(22,163,74,0.25)] scale-105 z-10 -translate-y-1' : 'hover:border-[#16a34a]/60 hover:shadow-lg hover:-translate-y-1'}`}
                >
                  {/* Standardized icon with unified green color */}
                  <div className="w-full flex justify-center">
                    <div className={`mb-4 flex items-center justify-center w-14 h-14 md:w-16 md:h-16 rounded-full bg-[#16a34a]/10 transition-all duration-200 ${isOpen ? 'scale-110 bg-[#16a34a]/20' : ''}`}>
                      <Icon className={`w-10 h-10 md:w-12 md:h-12 transition-transform duration-200 ${isOpen ? 'text-[#16a34a] scale-110' : 'text-[#16a34a]'}`} />
                    </div>
                  </div>
                  <h3 className="text-lg md:text-xl font-bold mb-2 md:mb-3 text-white text-center md:min-h-[56px] flex items-center justify-center px-1 md:px-2">
                    {benefit.headline}
                  </h3>
                  <p className="text-gray-300 text-sm md:text-base text-center md:min-h-[72px] px-1 md:px-2">
                    {benefit.copy}
                  </p>
                  {/* Card expandible */}
